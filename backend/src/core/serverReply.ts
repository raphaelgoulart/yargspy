...
export const codeMap = {
  ...
  err_syntax_json_body: [
    400,
    "Response body JSON has a syntax error: {{additionalMessage}}",
  ],
  err_auth_required: [401, "No authorization string found on request headers"],
  err_invalid_auth: [
    401,
    "The provided authorization is not valid. Please logout this session, validate a new login and try again",
  ],
  ...
  // user/register
  err_user_register_no_body: [
    400,
    "No body response provided for user registering route",
  ],
  err_user_register_no_username: [
    400,
    "No username provided for user registering",
  ],
  err_user_register_no_password: [
    400,
    "No password provided for user registering",
  ],
  err_user_register_username_toosmall: [
    400,
    "Provided username must have at least 3 characters",
  ],
  err_user_register_username_toobig: [
    400,
    "Provided username can't have more than 32 characters",
  ],
  err_user_register_password_toosmall: [
    400,
    "Provided password must have at least 8 characters",
  ],
  err_user_register_password_toobig: [
    400,
    "Provided password can't have more than 32 characters",
  ],
  err_user_register_password_nolowercase: [
    400,
    "Provided password must contain at least one lowercase character",
  ],
  err_user_register_password_nouppercase: [
    400,
    "Provided password must contain at least one uppercase character",
  ],
  err_user_register_password_nonumber: [
    400,
    "Provided password must contain at least one numeric digit",
  ],
  err_user_register_password_nospecialchar: [
    400,
    "Provided password must contain at least one special character",
  ],
  err_user_register_duplicated_username: [
    409,
  ...
  // user/login
  err_user_login_no_body: [
    400,
    "No body response provided for user login validation route",
  ],
  err_user_login_no_username: [
    400,
    "No username provided for user login validation",
  ],
  err_user_login_no_password: [
    400,
    "No password provided for user login validation",
  ],
  err_login_password_validation: [
    401,
    "The provided password and the registered user's password don't match",
  ],
  err_login_user_notfound: [
    400,
 ...
 */
export const serverReply = (
  reply: FastifyReply,
  codeOrMessage: LiteralUnion<ReplyCodeNames, string> | DirectMessage,
