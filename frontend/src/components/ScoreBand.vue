<template>
  <tr class="border-t-1 border-gray-800">
    ...
    <td
      scope="col"
      class="font-medium"
      :title="score.modifiers.length ? 'At least one player used a modifier' : ''"
    >
      ...
      <RouterLink :to="{ name: 'player', params: { username: score.uploader.username } }">
        {{ score.uploader.username }}</RouterLink
      >'s band<span v-if="score.modifiers.length">*</span>
      <span class="text-xs">
        ({{ score.childrenScores.length }}
        {{ score.childrenScores.length > 1 ? 'players' : 'player' }})</span
      >
    </td>
    <td scope="col">
      ...
      <td colspan="8" class="px-3 pt-1 pb-2 text-xs bg-gray-800">
        <table class="w-full text-sm text-left rtl:text-right text-gray-400">
          <thead class="text-xs text-gray-400">
            <tr>
              <th scope="col" class="pl-3 py-2">Score</th>
          ...
          <tbody class="text-sm text-slate-300">
            <ScoreBandMember
              v-for="score in score.childrenScores"
              :key="score._id"
              :score="score"
              :difficulty="instruments[score.instrument][score.difficulty!]"
            />
          </tbody>
        </table>
      </td>
