    ...
    <AnimatedComponent condition={isActivated}>
      <MotionSection {...createAnimation({ opacity: true })} className="!absolute min-h-full w-full origin-top">
        ...
        <div className="mb-2 border-b border-neutral-700 pb-2">
          ...
          {(() => {
            const { code, message, statusCode, statusFullName, statusName, requestTime, token }: DebugUserLoginResponseDecorators = lastRequest ?? { code: 'off', message: t('debug_make_first_req_message'), statusCode: 0, statusFullName: '000 Off', statusName: 'Off', requestTime: 0 }
            return (
              <>
