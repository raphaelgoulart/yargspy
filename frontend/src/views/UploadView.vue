        ...
        <div v-if="songDataRequired">
          <TheAlert color='yellow' class="text-center mt-4">
            <div>
              ...
              <span class="align-middle ml-1"><b>WARNING:</b> This song isn't available on our database yet, so<br />we need some more information for replay validation purposes.</span>
            </div>
          </TheAlert>
          <div class="mb-4">
            <FormInputFile name="chartFile" label="notes.mid/notes.chart file" :required="songDataRequired" :disabled="!songDataRequired" accept=".mid,.chart" />
          </div>
          <div class="mb-2">
            <FormInputFile name="songDataFile" label="song.ini/.dta file" required accept=".ini,.dta" />
          </div>
        </div>
